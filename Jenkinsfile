...
def DOCKER_IMAGE='skhappycampus/skhappycampus-auth'
...
podTemplate(label:label,
    ...
    node(label) {
        ...
        stage('DEPLOY') {
            container('kubectl') {
                kubeCmd.apply file: 'k8s/skhappycampus-auth-service.yaml', namespace: K8S_NAMESPACE
                yaml.update file: 'k8s/skhappycampus-auth-deployment.yaml', update: ['.spec.template.spec.containers[0].image': "${HARBOR_REGISTRY}/${DOCKER_IMAGE}:${BUILD_NUMBER}"]
                kubeCmd.apply file: 'k8s/skhappycampus-auth-deployment.yaml', wait: 1000, recoverOnFail: false, namespace: K8S_NAMESPACE
            }
        }
